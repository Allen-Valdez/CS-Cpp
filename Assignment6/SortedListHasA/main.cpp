...
void displayScore(SortedListInterface<int> *intList)
{
   int length  = intList->getLength();
   cout << "***Scoreboard*** " << endl;
   // for (int pos = 1; pos <= length; pos++)
   // {
   //    cout << intList->getEntry(pos) << " ";
   // } // end for
   // cout << "TEST" << endl;
   // cout << endl;
...
int main()
{
   ...
   cout << "ASSIGNMENT 4: HIGH SCORES" << endl;

   int num{1};
   SortedListInterface<int> *intList = new SortedListHasA<int>();
   while (num > 0)
   {
      cout << "Please enter the score (Enter negative value to exit the program)" << endl;
      cin >> num;
      if (num < 0)
      {
         break;
      ...
      {
         intList->insertSorted(num);
         displayScore(intList);
         cout << endl
            << endl;
      }
   }
